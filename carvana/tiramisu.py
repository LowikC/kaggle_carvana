# from https://github.com/titu1994/Fully-Connected-DenseNets-Semantic-Segmentation/blob/master/densenet_fc.py
import warnings
...
def __conv_block(ip, nb_filter, bottleneck=False, dropout_rate=None, weight_decay=1E-4):
    ...
    if bottleneck:
        ...
        x = Convolution2D(inter_channel, (1, 1), kernel_initializer='he_uniform',
    ...
    x = Convolution2D(nb_filter, (3, 3), kernel_initializer="he_uniform",
                      padding="same", use_bias=False,
                      kernel_regularizer=l2(weight_decay))(x)
    if dropout_rate:
        x = Dropout(dropout_rate)(x)
...
def __transition_up_block(ip, nb_filters, type='upsampling', output_shape=None, weight_decay=1E-4):
    ...
    if type == 'upsampling':
        ...
        x = AtrousConvolution2D(nb_filters, (3, 3), activation="relu",
                                kernel_regularizer=l2(weight_decay),
                                use_bias=False, atrous_rate=(2, 2),
                                kernel_initializer='he_uniform')(ip)
    else:
        x = Deconvolution2D(nb_filters, (3, 3), output_shape, activation='relu',
                            padding='same',
...
def __create_fcn_dense_net(nb_classes, img_input, include_top, nb_dense_block=5, growth_rate=12,
    ...
    if include_top:
        x = Convolution2D(nb_classes, (1, 1), activation='linear',
